...
class Player
    def hand
      ...
      input_hand = gets.chomp # 変数「input_hand」にプレイヤーの入力値を代入します。# ヒント：getsメソッドについて調べてみましょう。
     
      # 「input_hand」が「0, 1, 2」のいずれかだと繰り返し処理を終了し、それ以外（アルファベットも含む）だと繰り返し処理を継続します。
     while true
        if ["0","1","2"].include?(input_hand)  # if 「input_hand」が「0, 1, 2」のいずれかの場合だった場合 # ヒント：include?メソッドについて調べてみましょう。 
            return input_hand.to_i # 「input_hand」をそのまま返す。# ヒント：戻り値を返して繰り返し処理を終了させたい場合、「return」を使用します。
        else  # else それ以外の場合
          puts "0~2の数字を選んでください"   # プレイヤーに「0〜2」を入力させる文章を表示させる。# puts "0〜2の数字を入力してください。"
          puts "0:グー, 1:チョキ, 2:パー"
...
class Janken
    def pon(player_hand, enemy_hand)
      janken = ["グー", "チョキ", "パー"] # 変数「janken」に["グー", "チョキ", "パー"]を代入します。
      puts "相手の手は#{janken[enemy_hand]}です" #「相手の手は#{相手の手}です。」と出力させます。
      puts "あなたの手は#{janken[player_hand]}" # Playerクラスの戻り値とEnemyクラスの戻り値からじゃんけんするロジックを作成します。
      puts"------------------------------------------"
